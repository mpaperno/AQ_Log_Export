	...
	currentTranslator(0)
{
	...
	connect(ui->checkBox_timestamp, SIGNAL(toggled(bool)), ui->checkBox_localtime, SLOT(setEnabled(bool)));
	connect(ui->radioButton_trigVal_opt02, SIGNAL(toggled(bool)), ui->spinBox_trigVal_lt, SLOT(setEnabled(bool)));
	connect(ui->radioButton_trigVal_opt01, SIGNAL(toggled(bool)), ui->spinBox_trigVal_gt, SLOT(setEnabled(bool)));
	connect(ui->checkBox_timestamp, SIGNAL(toggled(bool)), ui->label_dateOfFlight, SLOT(setEnabled(bool)));
	connect(ui->checkBox_timestamp, SIGNAL(toggled(bool)), ui->dateEdit_logDate, SLOT(setEnabled(bool)));
...
void AQLogExporter::autoExportFileName() {
	QString fileName = QDir::toNativeSeparators(ui->lineEdit_inputFile->text());
	QFileInfo fi(fileName);
...
void AQLogExporter::readSettings() {
	...
	ui->lineEdit_inputFile->setText(settings.value("LastLogFile", "").toString());
//	ui->lineEdit_outputFile->setText(settings.value("LastExportFile", "").toString());
	...
	ui->buttonGroup_trigChanValues->button(settings.value("ChannelValueTypeId", 1).toInt())->setChecked(true);
...
void AQLogExporter::writeSettings() {
	...
	settings.setValue("LastLogFile", ui->lineEdit_inputFile->text());
	settings.setValue("LastExportFile", ui->lineEdit_outputFile->text());
...
void AQLogExporter::on_toolButton_autoExportFile_clicked()
{
	autoExportFileName();
}
